...
//  { name: 'partner', type: String, defaultValue:"./keys/Alex_pub.hex"},
  { name: 'tx', type: String},
]
...
var key_bin = new Buffer(key_hex, 'hex')
...
//var PrPriv = new Buffer(fs.readFileSync(opts.partner).toString().slice(2,66),'hex')
//var PrAddr='0x'+eth_util.pubToAddress(eth_util.privateToPublic(PrPriv)).toString('hex')
...
// challengeFinish(uint32 id,uint256 vA, uint256 vB,uint8 v,bytes32 r,bytes32 s)
...
var tx = new Tx({
    ...
    to: address,
    value: '0x0',
    data: instance.challengeFinish.getData(id,vA,vB, v,r,s)
});
tx.sign(MyPriv);
